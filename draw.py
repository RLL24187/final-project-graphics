...
def generate_cylinder(cx, cy, cz, radius, height, step):
    ...
    for layer in range(layer_start, layer_stop):
        for circle in range(circ_start, circ_stop):
            ...
            x = math.cos(2*math.pi * circ) * radius + cx;
            y = dy * layer + cy;
            z = math.sin(2*math.pi * circ) * radius + cz;
...
def add_cylinder( polygons, cx, cy, cz, radius, height, step):
    ...
    while i < step - 1:
        add_polygon(polygons,
                    ...
                    points[offset + i + 2][1],
                    points[offset + i + 2][2])
        print("Added polygon : " + str(len(points) - 1) + ", " + str(offset + i + 1) + ", " + str(offset + i + 2))
        i += 1
    add_polygon(polygons,
                top_center[0],
                top_center[1],
                ...
                points[offset + 1][1],
                points[offset + 1][2])
    print("Added polygon : " + str(len(points) - 1) + ", "+ str(offset + i + 1) + ", " + str(offset + 1))
...
def add_wedge( polygons, x, y, z, width, height, depth ):
    x1 = x + width
    y1 = y + height
    z1 = z - depth
    ...
    #top
    add_polygon(polygons, x, y, z1, x, y, z, x1, y1, z)
    add_polygon(polygons, x, y, z1, x1, y1, z, x1, y1, z1)
